...
enum LVAL_TYPE { LVAL_ERR, LVAL_NUM,  LVAL_SYM, LVAL_STR, 
...
char* ltype_name(enum LVAL_TYPE type) {
  char* types[] = { "LVAL_ERR", "LVAL_NUM",  "LVAL_SYM", "LVAL_STR", 
       "LVAL_FUN", "LVAL_SEXPR", "LVAL_QEXPR", "LVAL_BOOL" };
...
lval* builtin_head(lenv* e, lval* a) {
  ...
  int expected_type = a->cell[0]->type;
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR || 
    a->cell[0]->type == LVAL_STR, 
    "Invalid argument type for Head function. "
    "Got %s, Allowed types are %s or %s",
    ltype_name(a->cell[0]->type),
    ltype_name(LVAL_QEXPR),
    ltype_name(LVAL_STR));


  LASSERT_TYPE("head", a, 0, expected_type);
  lval* v;
...
lval* builtin_tail(lenv* e, lval* a) {
  ...
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR || 
    a->cell[0]->type == LVAL_STR, 
    "Invalid argument type for Tail function. "
    "Got %s, Allowed types are %s or %s",
    ltype_name(a->cell[0]->type),
    ltype_name(LVAL_QEXPR),
    ltype_name(LVAL_STR));
  LASSERT_TYPE("tail", a, 0, expected_type);
  ...
  switch (a->cell[0]->type) {
    ...
    case LVAL_STR:
      v = lval_take(a, 0);
      LASSERT(v, strlen(v->str) != 0,
      "Function 'tail' passed empty string");
      if (strlen(v->str) > 1) {
        char* str_tail = &(v->str)[1];
        memmove(v->str, str_tail, strlen(str_tail));
        v->str[strlen(str_tail)] = '\0';
      }
      break;
  }
  return v;
...
lval* builtin_join(lenv* e, lval* a) {
  ...
  LASSERT(a, a->cell[0]->type == LVAL_QEXPR || 
    a->cell[0]->type == LVAL_STR, 
    "Invalid argument type for Join function. "
    "Got %s, Allowed types are %s or %s",
    ltype_name(a->cell[0]->type),
    ltype_name(LVAL_QEXPR),
    ltype_name(LVAL_STR));
  ...
  for(int i = 0; i < a->count; i++) {
    LASSERT_TYPE("join", a, i, expected_type);
  }
...
BOOL lval_cmp(lval* x, lval* y) {
  ...
  switch(x->type){
    ...
    case LVAL_SYM:
      ...
      break;
    case LVAL_STR:
      val = LGET_BTYPE(strcmp(x->str, y->str) == 0);
      break;
    case LVAL_FUN:
